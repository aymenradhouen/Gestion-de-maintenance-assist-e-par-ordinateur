...
{
	$eid=$_REQUEST["eid"];
	$req=mysql_query("select * from moniteurs where id='$eid'");
...
if(isset($_REQUEST["enregistrer"]))
{
  ...
  mysql_query("update moniteurs set nom='$nom',type='$type',fabriquant='$fabriquant',modele='$modele',lieu='$lieu',affectation='$affectation',num_serie='$num_serie',code_inv='$code_inv',caracteristiques='$d',taille='$taille',date_achat='$date_achat',commentaires='$commentaires' where id='$eid' ");
...
<table border="0" class="tab1">
	...
	<tr>
										 ...
										 <td><select name="type">
											 ...
											 <?php
											 $re ="select type from type_moniteurs";
	...
	<tr>
                      ...
                      <td><select name="fabriquant">
                        ...
                        <?php
                        $re ="select fabriquant from fabriquant_moniteurs";
												$rq = mysql_query($re) or die(mysql_error());
								  			 while($ET=mysql_fetch_assoc($rq))
                        {
	...
	<tr>
                      ...
                      <td><select name="modele">
                        ...
                        <?php
                        $re ="select modele from modele_moniteurs";
												$rq = mysql_query($re) or die(mysql_error());
								  			 while($ET=mysql_fetch_assoc($rq))
                        {
	...
	<tr>
										 <td>Taille :</td>
										 <td><input type="text" name="taille" value="<?php echo $rs['taille'] ?>"></td>
</tr>
	<tr>
										 <td>Créer le :</td>
