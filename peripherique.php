...
if(isset($_REQUEST["supprimer"])&&!empty($_REQUEST["supprimer"]))
{
  if(!empty($_REQUEST["delid"]))
  {
  $delid=$_REQUEST["delid"];
  $a=implode(",",$delid);
  mysql_query("delete from peripheriques where id in ($a)");
...
$req="select * from peripheriques order by id";
...
<body>
  ...
  ?>
<a class="link" href="./ajouter/traiter.php?tid=<?php echo($exx1['id'])?>">Traiter cet equipement</a>
  <?php
}
} else {
...
?>
<a class="link" href="./ajouter/ajouter_demande.php?pid=<?php echo($did)?>">Ajouter une demande pour cet equipement</a>
<?php
...
<h1 align="">Peripheriques :</h1>
<?php if($_SESSION['type'] == "Administrateur du parc" || $_SESSION['type'] == "Administrateur"){ ?>
<form method="post" action="ajouter/ajouter_peripherique.php">
    ...
    <tr>
      ...
      <td><?php echo($ET['commentaires'])?></td>
              ...
              <?php if($_SESSION['type'] == "Administrateur du parc" || $_SESSION['type'] == "Administrateur"){ ?>
               ...
               <?php } ?>
               <?php if($_SESSION['type'] == "Technicien" || $_SESSION['type'] == "Administrateur"){ ?>
               <td><a class="link" href="peripherique.php?did=<?php echo($ET['modele'])?>">Intervention</a></td>
               <?php } ?>
    </tr>
    <?php } ?>
