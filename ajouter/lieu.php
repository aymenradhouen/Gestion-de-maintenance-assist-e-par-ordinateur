  ...
  if(!empty($_POST['lieu']))
   {
     ...
     $req ="insert into lieu_aff VALUES ('$id','$lieu')";
     mysql_query($req);
     header('Location: lieu.php');
   }
   else echo "<p class='faux'>Champs requis ne sont pas remplis</p>";
}
$req = "select * from lieu_aff order by id";
$rs=mysql_query($req);

if(isset($_REQUEST["delid"]))
{
  $delid=$_REQUEST["delid"];
  mysql_query("delete from lieu_aff where id='$delid'");
  header('Location: lieu.php');
}
...
<h1 align="">Ajouter un lieu :</h1><br />
        ...
        <table class="tab" border="1" width="40%" align="center">
           ...
           <tr>
             <td><?php echo($ET['lieu'])?></td>
               <td><a class="link" href="lieu.php?delid=<?php echo($ET['id'])?>">Supprimer</a></td>
           </tr>
           <?php } ?>
