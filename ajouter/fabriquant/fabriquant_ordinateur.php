  ...
  if(!empty($_POST['fabriquant']))
   {
     ...
     $req ="insert into fabriquant_ordinateurs VALUES ('$id','$fabriquant')";
     mysql_query($req);
     header('Location: ../ajouter_ordinateur.php');
   }
   else echo "<p class='faux'>Champs requis ne sont pas remplis</p>";
}
$req = "select * from fabriquant_ordinateurs order by id";
$rs=mysql_query($req);

if(isset($_REQUEST["delid"]))
{
  $delid=$_REQUEST["delid"];
  mysql_query("delete from fabriquant_ordinateurs where id='$delid'");
  header('Location: fabriquant_ordinateur.php');
}
...
<h1 align="">Ajouter un type d'ordinateur :</h1><br />
      ...
      <table class="tab" border="1" width="40%" align="center">
         ...
         <tr>
           <td><?php echo($ET['fabriquant'])?></td>
             <td><a href="fabriquant_ordinateur.php?delid=<?php echo($ET['id'])?>">Supprimer</a></td>
         </tr>
         <?php } ?>
