  ...
  if(!empty($_POST['modele']))
   {
     ...
     $req ="insert into modele_materielres VALUES ('$id','$modele')";
     mysql_query($req);
     header('Location: ../ajouter_materielres.php');
   }
   else echo "<p class='faux'>Champs requis ne sont pas remplis</p>";
}
$req = "select * from modele_materielres order by id";
$rs=mysql_query($req);

if(isset($_REQUEST["delid"]))
{
  $delid=$_REQUEST["delid"];
  mysql_query("delete from modele_materielres where id='$delid'");
  header('Location: modele_materielres.php');
}
...
<h1 align="">Ajouter un modele de materiel reseau :</h1><br />
<form method="post" action="">
      ...
      <table class="tab" border="1" width="40%" align="center">
         <tr><th>Modele</th><th></th></tr>

         <?php while($ET=mysql_fetch_assoc($rs)) { ?>
         <tr>
           <td><?php echo($ET['modele'])?></td>
             <td><a href="modele_materielres.php?delid=<?php echo($ET['id'])?>">Supprimer</a></td>
         </tr>
         <?php } ?>
