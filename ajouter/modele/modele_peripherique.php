  ...
  if(!empty($_POST['modele']))
   {
     ...
     $req ="insert into modele_peripheriques VALUES ('$id','$modele')";
     mysql_query($req);
     header('Location: ../ajouter_peripherique.php');
   }
   else echo "<p class='faux'>Champs requis ne sont pas remplis</p>";
}
$req = "select * from modele_peripheriques order by id";
$rs=mysql_query($req);

if(isset($_REQUEST["delid"]))
{
  $delid=$_REQUEST["delid"];
  mysql_query("delete from modele_peripheriques where id='$delid'");
  header('Location: modele_peripherique.php');
}
      ...
      <table class="tab" border="1" width="40%" align="center">
         ...
         <tr>
           <td><?php echo($ET['modele'])?></td>
             <td><a href="modele_peripherique.php?delid=<?php echo($ET['id'])?>">Supprimer</a></td>
         </tr>
         <?php } ?>
