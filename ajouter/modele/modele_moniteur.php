  ...
  if(!empty($_POST['modele']))
   {
    ...
    $modele=mysql_real_escape_string($_POST['modele']);
     ...
     $req ="insert into modele_moniteurs VALUES ('$id','$modele')";
     mysql_query($req);
     header('Location: ../ajouter_moniteur.php');
   }
   else echo "<p class='faux'>Champs requis ne sont pas remplis</p>";
}
$req = "select * from modele_moniteurs order by id";
$rs=mysql_query($req);

if(isset($_REQUEST["delid"]))
{
  $delid=$_REQUEST["delid"];
  mysql_query("delete from modele_moniteurs where id='$delid'");
  header('Location: modele_moniteur.php');
}
...
<h1 align="">Ajouter un modele de moniteur :</h1><br />
      ...
      <table class="tab" border="1" width="40%" align="center">
         ...
         <tr>
           <td><?php echo($ET['modele'])?></td>
             <td><a href="modele_moniteur.php?delid=<?php echo($ET['id'])?>">Supprimer</a></td>
         </tr>
         <?php } ?>
