  ...
  if(!empty($_POST['type']))
   {
     ...
     $req ="insert into type_consommables VALUES ('$id','$type')";
     mysql_query($req);
     header('Location: ../ajouter_consommable.php');
   }
   else echo "<p class='faux'>Champs requis ne sont pas remplis</p>";
}
$req = "select * from type_consommables order by id";
$rs=mysql_query($req);

if(isset($_REQUEST["delid"]))
{
  $delid=$_REQUEST["delid"];
  mysql_query("delete from type_consommables where id='$delid'");
  header('Location: type_consommable.php');
}
...
<h1 align="">Ajouter un type de consommable :</h1><br />
        ...
        <table class="tab" border="1" width="40%" align="center">
           ...
           <tr>
             <td><?php echo($ET['type'])?></td>
               <td><a href="type_consommable.php?delid=<?php echo($ET['id'])?>">Supprimer</a></td>
           </tr>
           <?php } ?>
