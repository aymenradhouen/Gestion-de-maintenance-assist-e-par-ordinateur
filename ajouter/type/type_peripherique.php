  ...
  if(!empty($_POST['type']))
   {
     ...
     $req ="insert into type_peripheriques VALUES ('$id','$type')";
     mysql_query($req);
     header('Location: ../ajouter_peripherique.php');
   }
   else echo "<p class='faux'>Champs requis ne sont pas remplis</p>";
}
$req = "select * from type_peripheriques order by id";
$rs=mysql_query($req);

if(isset($_REQUEST["delid"]))
{
  $delid=$_REQUEST["delid"];
  mysql_query("delete from type_peripheriques where id='$delid'");
  header('Location: type_peripherique.php');
}
      ...
      <table class="tab" border="1" width="40%" align="center">
         ...
         <tr>
           <td><?php echo($ET['type'])?></td>
             <td><a href="type_peripherique.php?delid=<?php echo($ET['id'])?>">Supprimer</a></td>
         </tr>
         <?php } ?>
