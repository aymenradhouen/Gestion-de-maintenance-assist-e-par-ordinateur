  ...
  if(!empty($_POST['type']))
   {
     ...
     $req ="insert into type_materielres VALUES ('$id','$type')";
     mysql_query($req);
     header('Location: ../ajouter_materielres.php');
   }
   else echo "<p class='faux'>Champs requis ne sont pas remplis</p>";
}
$req = "select * from type_materielres order by id";
$rs=mysql_query($req);

if(isset($_REQUEST["delid"]))
{
  $delid=$_REQUEST["delid"];
  mysql_query("delete from type_materielres where id='$delid'");
  header('Location: type_materielres.php');
}
...
<h1 align="">Ajouter un type de materiel reseau :</h1><br />
      ...
      <table class="tab" border="1" width="40%" align="center">
         ...
         <tr>
           <td><?php echo($ET['type'])?></td>
             <td><a href="type_materielres.php?delid=<?php echo($ET['id'])?>">Supprimer</a></td>
         </tr>
         <?php } ?>
